...
import (
    ...
    futuresstream "github.com/btcnash/go-binance/v2/futures/stream"
)
...
Stream sessions proactively recycle the physical WebSocket at `23h50m` by default. Set `DisableRotation` to true or provide `ConnectionOptions.MaxConnectionAge` for a custom policy. Reconnection restores the current desired subscription set before the logical session returns to Ready.
