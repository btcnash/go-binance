...
import (
    ...
    gorilladialer "github.com/btcnash/go-binance/v2/common/websocket/managed/gorilla"
)
...
`SendTextOnGeneration` lets an upper protocol register work against a specific
physical generation and reject the write if reconnect has already replaced that
generation. M4 uses it to prevent a request future registered for one socket
from being sent to another socket during a reconnect race.
...
## Maximum physical connection age

Set `Options.MaxConnectionAge` to proactively recycle a physical connection. The logical connection remains active and reconnects with a new generation. Futures Stream and Private sessions default this value to `23h50m`; zero disables it at the managed-core level.
